...
A $\textbf{martingale}$ is a mathematical model for a
sequence of fair gambles which has found many
applications in both theoretical and applied
probability. (In particular, martingales play an
important role in the mathematics of investing and
other areas of mathematical finance --- so knowing
about them can actually make you money!) One of the
most important facts about martingales is that under
fairly mild assumptions they converge (almost surely,
or in $L_1$ or according to some other notion of
convergence). This is made precise in a family of
important results known as \emph{martingale convergence
theorems}. Our goal in the next few chapters will be
to develop the basic theory of martingales and its
applications and prove some of the martingale
convergence theorems.

We start our study of martingales with a motivating
example: a famous experiment known as
$\textbf{Pólya's urn experiment}$. In this model, an
urn originally contains $a$ white balls and $b$ black
balls. The experimenter samples a uniformly random
ball from the urn, examines its color, then puts the
ball back and adds another ball of the same color;
this is repeated to infinity. Let $X_n$ denote the
number of white balls in the urn after the $n$th
step. Clearly $X_0 = a$ and the distribution of
$X_{n+1}$ can be expressed most naturally by
conditioning on $X_n$, namely
...
It turns out that it is not too difficult to find an
unconditional formula for the probability distribution
of $X_n$. Let $I_n$ be the indicator random variable
of the event that in the $n$th sampling step a white
ball was drawn. First, an amusing observation is that
the probability of observing a particular sequence of
white/black samples in the first $n$ sampling steps
is only dependent on the number of white and black
balls. That is, for any sequence
$(x_1,x_2,\ldots,x_n)\in \{0,1\}^n$ the probability
$\mathrm{P}(I_1 = x_1, \ldots, I_n = x_n)$ only
depends on $k=\sum_{j=1}^n x_j$. To see this, note
that if at a given stage $t$ of the experiment the
urn contained $A$ white and $B$ black balls, the
probability to draw ``white then black'' in the next
two steps would be
...
which is clearly equal to the probability of drawing
``black then white,'' given by

\[
\frac{B}{t+a+b}\cdot \frac{A}{t+a+b+1}.
\]

Thus, permuting the 1's and 0's in the sequence
$(x_1,\ldots,x_n)$ has no effect on the
probability\footnote{A sequence of r.v.'s with this property is called
$\textbf{exchangeable}$. There is an important result
about such sequences (that we will not talk about
here) called $\textbf{De-Finetti's theorem}$, which
you might want to read about.}. It follows that for
any such $(x_1,\ldots,x_n)$ with $\sum x_j=k$ we have
...
Therefore the probability of having $p$ white balls
after $n$ steps is
...
for $a \le p \le n+a$. This formula is so explicit
that it is easy to analyze the distribution of $X_n$
and show for example the convergence in distribution

\begin{equation*} \tag{3}\label{eq:polya-beta-dist}
\frac{X_n}{n+a+b} \implies \textrm{Beta}(a,b).
\end{equation*}

That is, the \emph{proportion} of white balls in the urn
converges to a limiting beta distribution.

\begin{xexercise}
Prove (\ref{eq:polya-beta-dist}).
\end{xexercise}

What about stronger notions of convergence such as
convergence in probability or almost sure convergence?
This seems like a harder question that requires
understanding more about the \emph{joint} distribution of
different $X_n$'s. It turns out that we also have
almost sure convergence, that is, the limit

\begin{equation*} \tag{4}\label{eq:polya-as-conv}
Y = \lim_{n\to\infty} \frac{X_n}{n+a+b}
\end{equation*}

exists almost surely, and that the main relevant fact
that guarantees that this convergence takes place is
that the proportions $M_n = X_n/(n+a+b)$ form a
martingale! To see what this means, note that
(\ref{eq:polya-conditional}) can be rewritten in the form
...
Thus, the value of $M_{n+1}$ can be either greater or
smaller than the value of $M_n$, but the amounts by
which it increases or decreases, weighted by the
respective probabilities of each of these events,
balance out, so that \emph{on average}, the value stays the
same:
...
The martingale property, which will immediately imply
the a.s.\ convergence (\ref{eq:polya-as-conv}) through the
use of one of the martingale convergence theorems, is
a generalization of this statement for arbitrary
sequences $(M_n)_{n=1}^\infty$. It is written in the
more abstract form:
...
In this equation, the quantity on the left represents
a new kind of random variable that we still haven't
discussed, a \emph{conditional expectation}. So, before
developing the theory of martingales we need a good
understanding of conditional expectations. This is the
...
$\mathbf{E}\left(X\,|\,Y\right)$ (``the conditional
...
In this case, if we know that $Y=y_j$ for some
...
For each $j$, this is a \emph{number}. It makes sense to
...
Let us try to think of a more conceptual way to look
...
and is a sub-$\sigma$-algebra of $\mathcal{F}$. For
...
Here, for each $1\le j\le n$ the quantity
...
so $\mathrm{P}(A) = 0$. Since $\epsilon$ was an
...
The most tricky part in defining conditional
...
There is an interesting intuitive interpretation to
...
Since $\sum_{n=1}^\infty 2^{-n}<\infty$, by the
...
This shows that $P$ is not absolutely continuous with
respect to $Q$.
\end{proof}
...
where $Z$ is some nonnegative random variable on the
...
As mentioned above, the ``if'' part of the theorem is
...
Another reason why it makes sense to think of $Z$ as
...
which holds for any random variable $X$ for which
$\mathbf{E}_P|X|<\infty$.
\end{xexercise}
...
Note that $Q$ is not a \emph{probability} measure; rather,
...
Thus, $Z$ satisfies the two properties (i)--(ii) in
...
so $Z$ satisfies the requirements that qualify it to
...
Thus $Z$, qualifies as the conditional expectation of
$X$ given $\mathcal{G}$.
\end{proof}
...
then continue as in the original proof. The
...
The idea here is that if $X$ is measurable with
...
First, if $X=\mathbf{1}_B$ for some $B\in\mathcal{G}$
then
...
so the claim is true. By linearity it follows that if
...
The problem of finding the $Y$ that minimizes this
...
Denote $W=Y-Z$. This is a $\mathcal{G}$-measurable
...
which is clearly minimized when (and only when) $Y=Z$
a.s.
\end{proof}

The above result has an interesting geometric
...
of sub-$\sigma$-algebras of $\mathcal{F}$. We refer
...
If instead of property M3.\ the sequence satisfies
the condition
...
where the notation $y\sim x$ means that $x,y$ are
...
The starting point $X_0$ of the walk can be a
...
so $(M_n)_{n=1}^\infty$ is a martingale with respect
...
The idea is that the $n$th random variable
...
A random variable $N$ taking values in
...
for each $k\ge 1$. It is easy to check that the
sequence
...
is a predictable sequence; it corresponds to the
...
We refer to $U_n$ as the number of
...
since $(H\bullet Y)$ would represent the profit made
...
exists almost surely and satisfies
$\mathbf{E}|X|<\infty$.
\end{thm}
...
denote the total number of $(a,b)$-upcrossings over
...
by the assumption on $\sup_n \mathbf{E}(X_n)_+$ and
...
has probability $0$. On the almost sure complementary
...
exists as a generalized r.v.\ taking values in
...
which also implies that $X<\infty$ a.s. Similarly,
note that
...
since $X_n$ is a submartingale, so again by Fatou's
lemma we have
...
which shows that $X>-\infty$ a.s. We have shown that
...
Elements of $L_p$ are considered only up to almost
...
For $p=\infty$ one may also define the space
...
where \textbf{essentially bounded} means that after
...
The space $L_p$ equipped with the norm
...
Since $L_p$ is actually a normed space, the theorem
...
The elements of $L_2$ are referred to as
...
It should be noted that the concept of convergence in
...
Note that the above theorem is only valid for $p>1$.
...
It is easy to see that a uniformly integrable family
...
If $(X_n)_{n=1}^\infty$ is a martingale and not just
...
In particular, if $A \in \mathcal{G}_\infty$ then we
have
...
(This last fact is sometimes called
...
of white balls in the urn is a martingale, which
...
Note that the almost sure martingale limit
...
corresponding to the assumption that each of the
...
The reason martingale theory is relevant to the
...
Since $E^c = \cap_{n=1}^\infty \{ Z_n>0 \}$, an
...
be the limit of the nonnegative martingale
...
To prove the final claim regarding the behavior when
...
The function $\varphi(z)$ is called the \textbf{generating
...
(in words: the $n$th functional iterate of $\varphi$
evaluated at $z=0$.)
\end{lem}
...
Using the lemma we see that if we are able to prove
that
...
the final part of Theorem \ref{thm-galton-watson} will
...
In fact, since $\mu>1$ there must be some $k\ge 2$
...
where $R_n$ represents the ``random rate of
...
where $a,b$ are two values in $(-1,\infty)$
...
Here, $A_n$ denotes the number of stock investment
...
as her expected return after $N$ time units of
...
the investor may conclude that $v_\textrm{fair}$ is
...
In finance, it is typical to think of this type of
...
(where as before $X_n = A_n S_n + V_n B_n$ denotes
...
where $F_n = (1+r)^{-n} A_n S_{n-1}$ and
...
Here, the sequence $(F_n)_n$ is predictable with
respect to the filtration $(\mathcal{G}_n)_n$, and
...
proving (\ref{eq:hedging-strategy-condition}). This
...
so that $Y_0 = x$ and $Y_N = (1+r)^{-N} (S_N-K)_+$,
...
Working our way backwards, we now define
...
Ergodic theory is a mathematical theory that evolved
out of the study of global properties of dynamical
systems. Here, we speak loosely of a
$\textbf{dynamical system}$ as consisting of a
$\textbf{phase space}$ $\Omega$ (a set, whose points
are the possible states of the system) together with
some $\textbf{dynamics}$, which are a notion of how
the state of the system evolves over time. Time may
flow continuously or in discrete steps. In the
simplest case of discrete-time dynamics, the dynamics
are encapsulated by a mapping $T:\Omega\to \Omega$.
We imagine that if at a given time the state of the
system is some point $\omega \in \Omega$, then in the
next time step it will be $T(\omega)$. (We assume
that the dynamics, i.e., the rules of evolution of the
system over time, are themselves unchanging over
time.)

The description of the dynamics in a continuous-time
dynamical system is more subtle; it consists of a
family $(T_s)_{s\ge 0}$ of maps, where for each
$s\ge 0$, $T_s:\Omega\to \Omega$ takes the current
state of the system $\omega\in \Omega$ and returns a
new point $\omega'=T_s(\omega)$ which represents the
state of the system $s$ time units into the future.
The maps therefore have to satisfy the conditions
...
i.e., the family $(T_s)_{s\ge 0}$ is a transformation
semigroup. In a context where the phase space has a
differentiable structure and the dynamics are a result
of solving a differential equation, the semigroup
$(T_s)_{s\ge0}$ is often called a $\textbf{flow}$.

Dynamical systems arise naturally in physics,
probability, biology, computer science (algorithmic
computations can often be interpreted as discrete-time
dynamical systems) and many other areas. To illustrate
the types of questions that ergodic theory deals with,
consider the example of
$\textbf{(mathematical) billiards}$: this is a
mathematical idealization of the game of billiards in
which a small ball is bouncing around without loss of
energy in some bounded and odd-shaped region of the
plane, being reflected off the walls; see Figure 3 for
two examples. The main question of ergodic theory can
be roughly formulated as follows:

\emph{If an observer watches the system for a long time,
starting from some arbitrary (random) initial state,
can the ideal statistics of the system be recovered?}
...
{\small Figure 3: Billiard dynamical systems: (a) The
``Bunimovich stadium''; (b) The ``Sinai billiard''
(source: Wikipedia)}
\end{center}

The question is formulated in a deliberately vague
way, but the idea behind ``ideal statistics'' is that
they are represented by some probability measure
$\mathrm{P}$ on the phase space $\Omega$ (equipped
with a suitable measurable structure $\mathcal{F}$)
that is compatible with both the way the ``arbitrary''
initial state of the system is chosen, and with the
action of the dynamics of the system (we shall make
these ideas more precise soon). The way the observer
will try to recover the measure $\mathrm{P}$ is as
follows: starting from the initial state $x_0$ one
gets a sequence of subsequent states

\[
x_0,\ \  x_1 = T(x_0),\ \  x_2 = T(T(x_0)),\ \  x_3=T^{3}(x_0), \ldots
\]

in the case of a discrete-time system, or a
one-parameter family of states

\[
x_s = T_s(x_0), \qquad (s\ge 0)
\]

for a continuous-time system (in both the discrete
and continuous cases this would be referred to as the
\textbf{orbit of $x_0$} under the dynamics). For a given
event $A\in \mathcal{F}$, the observer computes the
$\textbf{empirical frequencies}$ of occurrence of $A$
in the orbit, namely
...
One might expect that in a typical situation, the
quantity $\mu_A^{(n)}(x_0)$ or its continuous-time
analogue $\mu_A^{(s)}(x_0)$ should converge (as $n$
or $s$ tend to infinity) to a limit that is a
constant and independent of $x_0$, except possibly
for some small set of ``badly-behaved'' initial states
$x_0$. If that is the case, we might denote this
limit by $\mathrm{P}(A)$ and say that it represents
the ``ideal'' statistics of the system.

A more general way of recovering the statistics of
the system is to look at $\textbf{observables}$,
which are measurable functions
$f:\Omega\to \mathbb{R}$ on the phase space (an
observable is the dynamical systems or physics
equivalent term for a random variable, really). For an
observable $f$ we can form the
$\textbf{ergodic average}$

\[
\mu_f^{(n)}(x_0) = \frac1n \sum_{k=0}^{n-1} f(x_k) =\frac1n \sum_{k=0}^{n-1} f(T^k(x_0)),
\]

(or the analogous continuous-time quantity, whose
form we leave to the reader to write down), and hope
that again the ergodic averages converge to a limit,
which is independent of $x_0$ and represents the
``ideal'' average value of the observable $f$,
denoted $\mathbf{E}(f)$ (in physics, usually this
would be denoted $\langle f \rangle$). By computing
this ideal average for many different observables we
can recover all the information on the probability
measure $\mathrm{P}$.

One can now ask whether the nice situation described
above actually happens in practice. Coming back to the
example of billiards, it is easy to see that for some
shapes of the billiard ``table'' one cannot hope to
recover any meaningful statistics for the system, for
what may be a trivial reason. For example, a
rectangular table has the property that the ratio of
the absolute values of the horizontal and vertical
components of the initial speed of the ball is always
preserved (equivalently, the quantity $|\tan(\alpha)|$
where $\alpha$ is the initial angle is preserved).
Thus, by observing the trajectory of a single ball we
have no hope of recovering any meaningful information
on the statistics of the system when started with a
ball for which the ``invariant'' quantity
$|\tan(\alpha)|$ is different. In this case we say
that the billiard dynamical system on a rectangular
domain is $\textbf{non-ergodic}$. Less trivially, an
ellipse-shaped billiard can also be shown to be
non-ergodic, because of a less obvious geometric
invariance property: it can be shown that an orbit
will not fill the entire ellipse but will have a
non-trivial envelope which is either a smaller
ellipse, a hyperbola, a closed polygon or a line (see
\href{http://cage.ugent.be/~hs/billiards/billiards.html}{http://cage.ugent.be/~hs/billiards/billiards.html},
and Figure 4).
...
On the other hand, in many cases, such as the domains
shown in Figure 3, it can be proved that the nice
situation exists, i.e., the billiard \emph{is}
$\textbf{ergodic}$ (we will define later what that
actually means). This is related (in a way that is
difficult to articulate precisely), to the emergence
of a kind of ``chaos'' -- i.e., the billiard ball
trajectories are erratic and irregular rather than
forming a nice pattern as in the trivial examples
discussed above. When ergodicity holds, the statistics
of the system can be recovered from the typical
trajectory of a single ball; in the case of billiards,
it turns out that these statistics are quite
interesting: the underlying measure $\mathrm{P}$ on
the phase space (which may be parametrized in terms of
three parameters $\phi, \theta, \ell$ --- see the
article \emph{What is the ergodic theorem?} by
G.~D.~Birkhoff, \emph{American Math. Monthly}, April 1942,
for the meaning of these quantities) takes the form

\[
\mathrm{P}(A) = \iiint_A \frac{\sin \theta}{\sin\theta_1} \,d\theta\,d\phi\,d\ell.
\]

Note that even when the system is ergodic, there may
be exceptional orbits from which one cannot recover
any statistics. For example, in the Bunimovich stadium
shown in Figure 3, a trajectory that starts in a
vertical direction starting in the rectangular area
bounded between the two semi-circles will be a
periodic vertical line. However, the key point is that
such trajectories are atypical examples that only
occur on a measure 0 set of the phase space.

It should also be noted that in any given example,
\emph{proving} that the ergodicity property holds may be
extremely difficult. In fact, the family of dynamical
systems (and even more restrictively billiard systems)
for which ergodicity has been proved rigorously is
quite limited, and in practical dynamical systems that
one encounters in physics or other applied areas
usually this is assumed without proof, as long as
there is a sufficiently strong intuition that allows
one to rule out a ``trivial'' reason why ergodicity
should fail to hold.

In the next few sections, we shall start developing
the basic ideas of ergodic theory in a more formal and
precise way. The key concept is of a
$\textbf{measure-preserving system}$, which is a
probability space together with a
$\textbf{measure-preserving map}$ representing the
dynamics of the system. The main result we will prove
is the fundamental result of ergodic theory, known as
$\textbf{Birkhoff's pointwise ergodic theorem}$. It
explains precisely the connection between the notion
of ergodicity and the ability to ``recover the
statistics of the system'' as illustrated above. We
shall also give some important examples and explain
why the study of ergodic theory is natural from the
point of view of probability theory, since one can
consider the Birkhoff ergodic theorem as a powerful
...
If $T$ is measure preserving, we say that the
probability measure $\mathrm{P}$ is \textbf{invariant under
$T$}.
\end{defn}

The condition (\ref{eq:measure-pres}) is sometimes
written in the form
...
Measure preserving systems are the fundamental
...
Note that in particular this implies that the
...
(sometimes denoted by $\R^\infty$) together with the
...
(i.e., the distribution measure of the
...
What about a two-sided shift map? One can consider a
...
One may check easily that Lemma
...
We saw that we can associate with any stationary
...
The measure preserving property implies that
...
The conclusion from the above discussion is that the
...
The verification that $(Y_n)_n$ is stationary is easy
...
That is, the conditional distribution of $X_{n+1}$
...
then the matrix $P = (p_{i,j})_{i,j=1}^d$ together
...
the linear-algebraic result of multiplying the row
...
Note that the $X_n$'s are identically distributed (in
...
$\textbf{circle rotation map}$
$R_\alpha:[0,1)\to[0,1)$, which rotates the circle by
...
We have seen before that the measure space $[0,1]$
...
where $k\ge 1$, $n_1,\ldots,n_k\in \N$ and $n_k>1$.
...
where $n_1,n_2,n_3\ldots \in \N$. The numbers
...
Gauss studied in 1812 the statistical distribution of
...
(where $\lfloor z \rfloor$ denotes as usual the
...
(where $\{z\}=z-\lfloor z\rfloor$ is the
...
Then the above comments show that the quotients
...
(Note that the range of $G$ is $[0,1)$ instead of the
...
The map $G$ preserves the $\textbf{Gauss measure}$
$\gamma$ on $(0,1)$, given by
...
The conjecture is that for any initial number $n_0$,
...
where $a_0,a_1,a_2,\ldots\in \{0,1\}$. It can be
...
$\textbf{Hamiltonian mechanics}$ is a formalism for
...
where $r>0$ is a parameter of the system. Here, $x$
...
with the convention that two events $A,B$ are
...
A sub-$\sigma$-algebra of $\mathcal{F}$ all of whose
...
and only if the probability measure $\mathrm{P}$
\emph{cannot} be represented in the form

\[
\mathrm{P} = \alpha Q_1 + (1-\alpha)Q_2,
\]

where $0<\alpha<1$ and $Q_1, Q_2$ are two distinct
...
To get a feel for this new concept, let us examine
...
is in $\sigma(X_{n+1},X_{n+2},\ldots)$. It follows
that
...
is a tail event, and hence has probability $0$ or $1$
...
by the strong law of large numbers (conditioned on
...
where $U\sim \operatorname{Beta}(a,b)$. Thus, we have
...
is an example of a nontrivial invariant set.
...
(The equation represents an equality in $L_2$, i.e.,
...
where we denote $d_n = c_n e^{2\pi in\alpha}$. Since
$A$ is invariant, i.e.,
...
geodesic flow on the $\textbf{modular surface}$
$\mathbb{H}/PSL(2,\mathbb{Z})$, the quotient of the
...
operator on $H$. Let $P$ be the orthogonal projection
...
(Equivalently, the sequence of operators
...
Note that (\ref{eq:vonneumann}) holds trivially for
...
On the other hand, one can verify that $v\in V^\perp$,
...
(in general, the orthogonal complement of the
...
which means that $Uw-w = 0$, i.e., $w\in V$. Since
...
The fact that $T$ is measure preserving implies that
$U_T$ is unitary:
...
Note also that the subspace $\operatorname{Ker}(U-I)$
...
For the proof, we start by proving a lemma, known as
...
or equivalently $X \ge S_{k+1} - M_n\circ T$. Since
...
and therefore, noting that on the event $\{M_n>0\}$,
...
(since $A$ is an invariant event). Note that the
...
Furthermore, the convergence is dominated, since
...
and since $|Y_M|\le M$ the bounded convergence
...
so, combining this with (\ref{eq:ergodic-trunc-l1}), this
...
Letting $M\to\infty$ finishes the proof, since
...
To prove this, note that $\{ n\alpha \}$ is simply
...
as $n\to\infty$. This would appear to be a weaker
...
It is easy to check that this map has the invariant
measure
...
In fact, this is a thinly disguised version of the
...
where the convergence follows by the equidistribution
...
\emph{Table: The (approximate) digit frequencies in
Benford's law.}
...
for a set of $x$'s that has measure $1$ with respect
...
What other quantities of interest can one compute for
...
We mention one additional and very beautiful limiting
...
where $n_1,n_2,\ldots$ are the quotients in the
...
which are rational numbers that become better and
...
hold. How fast should we expect this sequence of
...
Note that (\ref{eq:cont-frac-entropy2}) and
...
with the convention that $0\log 0 = 0$. The logarithm
...
where $X$ is a random variable representing a source
...
This function is concave, has the symmetry
...
with equality holding if and only the two vectors are
equal.
\end{lem}
...
It is obvious that any prefix code is uniquely
...
By the law of large numbers, this quantity says how
...
For the second part, for each $1\le k\le d$ let
...
While the noiseless coding theorem clearly indicates
...
where the sequences in $T$ and $E$ are called the
...
and denote $S_n = \sum_{j=1}^n Z_j$. By the weak law
...
Call the event on the left-hand side $B$. This is an
...
for some set $T\subset A^N$ of sequences. This will
...
But $\prod_{n=1}^N p_{j_n}$ is exactly
...
The implication of the theorem is that since the
...
In information theory the quantity $H_n$ is often
...
The proof follows by induction by applying the result
...
I.e., $H(X\,|\,Y)$ is the average of the entropies of
...
We refer to $H$ in (\ref{eq:entropy-ergodic}) as \textbf{the
...
(Here, $H(X_1,\ldots,X_n)$ refers to the entropy of
...
Since $\sum_n e^{-n\epsilon}<\infty$, the first
...
where we denote $L(p) = p\log_2 p$ and
...
Since $L(\cdot)$ is a bounded continuous function on
...
Of course, the limit of $H_n$ is $H$, so we have
...
for the source entropy. Furthermore, this expression
...
where we adopt the notation (in the same vein as
...
and use this to write the right-hand side of
...
which is the same as the right-hand side of
...
Furthermore, one may verify without much difficulty
...
This fact bears some resemblance to the claim
...
So, we are in the situation described in Lemma
...
holds. Since $H_k \searrow H$, the inequalities
...
The idea in this definition is that
...
Combining it with an application of the ergodic
...
This again bears a resemblance to
...
which proves (\ref{eq:smb-second-half}) and thus finishes
the proof.
\end{proof}
...
We conclude this chapter with some examples of
...
Note that this stationary Markov chain has the same
...
One way of interpreting this fact is that, as we
...
i.e., on the average each continued fraction quotient
