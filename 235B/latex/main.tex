...
\noindent It turns out that it is not too difficult to find an
...
\noindent which is clearly equal to the probability of drawing
...
\noindent Thus, permuting the 1's and 0's in the sequence
...
\noindent Therefore the probability of having $p$ white balls
after $n$ steps is
...
\noindent for $a \le p \le n+a$. This formula is so explicit
...
\noindent That is, the \emph{proportion} of white balls in the urn
converges to a limiting beta distribution.

\begin{xexercise}
Prove \eqref{eq:polya-beta-dist}.
\end{xexercise}

\noindent What about stronger notions of convergence such as
...
\noindent exists almost surely, and that the main relevant fact
...
\noindent Thus, the value of $M_{n+1}$ can be either greater or
...
\noindent The martingale property, which will immediately imply
the a.s.\ convergence \eqref{eq:polya-as-conv} through the
...
\noindent In this equation, the quantity on the left represents
...
then the random variable $Z$ in \eqref{eq:abs-cont-p-q}
...
\eqref{eq:abs-cont-p-q} can be rewritten as an intuitive
...
\eqref{eq:abs-cont-change-var} generalizes to the identity
...
\begin{proof}[Proof of existence in \texorpdfstring{\hyperref[thm-cond-ext]{Theorem~\ref*{thm-cond-ext}}}{Theorem}]
...
then \eqref{eq:condexp-x-times-y} still holds. Now, assume
...
theorem, it follows that \eqref{eq:condexp-x-times-y}
holds also in this case. Finally, for general $X,Y$
one proves \eqref{eq:condexp-x-times-y} by splitting $X$
...
\begin{proof}
By \hyperref[thm-marting-transform]{Theorem~\ref*{thm-marting-transform}},
...
\begin{proof}
Define $Y_m = a+(X_m-a)_+$. By \texorpdfstring{\hyperref[cor-positive-part]{Corollary~\ref*{cor-positive-part}}}{Corollary}, $(Y_m)_m$ is a submartingale.
...
additional nonnegative profit. Finally, by \texorpdfstring{\hyperref[thm-marting-transform]{Theorem~\ref*{thm-marting-transform}}}{Theorem},
...
\begin{xexercise}
Use \eqref{eq:levy-zero-one} to give a new proof of
...
$M_n = S_{N\wedge n}$. By \texorpdfstring{\hyperref[thm-stopping-mart]{Theorem~\ref*{thm-stopping-mart}}}{Theorem}, the sequence $(M_n)_{n=1}^\infty$
is a martingale with respect to the filtration
$\mathcal{G}_n = \sigma(X_1,\ldots,X_n)$.
Furthermore, $M_n \ge -1$ for all $n$, so by
\hyperref[mart-conv-cor]{Corollary~\ref*{mart-conv-cor}}, the limit
...
\begin{proof}[Proof of parts 1 and 2 of \texorpdfstring{\hyperref[thm-galton-watson]{Theorem~\ref*{thm-galton-watson}}}{Theorem}]
...
\hyperref[mart-conv-cor]{Corollary~\ref*{mart-conv-cor}}. In the case when $\mu=1$,
...
\noindent the final part of \hyperref[thm-galton-watson]{Theorem~\ref*{thm-galton-watson}} will
...
\begin{proof}[Proof of part 3 of \texorpdfstring{\hyperref[thm-galton-watson]{Theorem~\ref*{thm-galton-watson}}}{Theorem}]
...
\eqref{eq:investment-strategy-initial} means that the
initial value $X_0$ of the investment account is $x$
units of currency, and
\eqref{eq:investment-strategy-rebalance} represents the
...
Equation \eqref{eq:discrete-black-scholes} is our
...
value $x$ as in \eqref{eq:investment-strategy-initial},
...
note that from \eqref{eq:investment-strategy-rebalance} we
have that
...
\noindent proving \eqref{eq:hedging-strategy-condition}. This
...
\begin{xexercise}
Prove \hyperref[lem-easy-martingale-rep]{Lemma~\ref*{lem-easy-martingale-rep}}.
\end{xexercise}
...
\eqref{eq:investment-strategy-rebalance} is satisfied,
...
\item The formula \eqref{eq:discrete-black-scholes} is written
...
\noindent i.e., the family $(T_s)_{s\ge 0}$ is a transformation
...
\noindent The question is formulated in a deliberately vague
...
\noindent in the case of a discrete-time system, or a
...
\noindent for a continuous-time system (in both the discrete
...
\noindent or, in the case of a continuous-time system

\[
\mu_A^{(s)}(x_0) = \frac{1}{s}\int_0^s \mathbf{1}_A(x_s)\,ds, \qquad (s\ge 0).
\]

\noindent One might expect that in a typical situation, the
...
\noindent (or the analogous continuous-time quantity, whose
...
\noindent On the other hand, in many cases, such as the domains
shown in Figure 3, it can be proved that the nice
situation exists, i.e., the billiard \emph{is}$\textbf{ergodic}$ (we will define later what that
...
\noindent Note that even when the system is ergodic, there may
...
\noindent The condition \eqref{eq:measure-pres} is sometimes
written in the form
...
\begin{xexercise}
Prove \hyperref[lem-shift-stationary]{Lemma~\ref*{lem-shift-stationary}}.
\end{xexercise}
...
stationary if the equation \eqref{eq:def-stationary} holds
...
\noindent One may check easily that \texorpdfstring{\hyperref[lem-shift-stationary]{Lemma~\ref*{lem-shift-stationary}}}{Lemma} remains true when replacing the
...
\eqref{eq:def-stationary} determines the joint
...
\begin{xexercise}
Prove \hyperref[lem-mps-stationary]{Lemma~\ref*{lem-mps-stationary}}
\end{xexercise}
...
meaning that the expression in \eqref{eq:markov-property}
...
\begin{xexercise}
Prove \hyperref[lem-gauss-measure]{Lemma~\ref*{lem-gauss-measure}}.
\end{xexercise}
...
\begin{xexercise}
Prove \hyperref[lem-logistic-invariant]{Lemma~\ref*{lem-logistic-invariant}}
\end{xexercise}
\end{enumerate}
...
\begin{xexercise}
Prove \hyperref[lem-invariant-sigmaalg]{Lemma~\ref*{lem-invariant-sigmaalg}}.
\end{xexercise}
...
be defined as in \eqref{eq:shift-equivariant}.
...
white balls in the urn (see \eqref{eq:polya-as-conv}) is
an invariant random variable. By
\eqref{eq:polya-beta-dist}, it is non-constant, which
...
short computation \eqref{eq:polyaurn-formula} can be
...
$(\tfrac12,\tfrac12)$ Bernoulli shift, so by \texorpdfstring{\hyperref[lem-iid-ergodic]{Lemma~\ref*{lem-iid-ergodic}}}{Lemma} above, the doubling map is ergodic.
...
\noindent Note that \eqref{eq:vonneumann} holds trivially for
...
\eqref{eq:vonneumann} holds for $v\in V+V'$. Next, we
...
Thus, \hyperref[thm-vonneumann]{Theorem~\ref*{thm-vonneumann}} applied to this setting
...
$X^* = (X-\epsilon)\mathbf{1}_{A}$. Applying \texorpdfstring{\hyperref[lem-ergodic-maximal]{Lemma~\ref*{lem-ergodic-maximal}}}{Lemma} to $X^*$ we get that
...
The convergence in \eqref{eq:birkhoffergodic1},
\eqref{eq:birkhoffergodic2} is also in $L_1$.
\end{thm}
...
\noindent so, combining this with \eqref{eq:ergodic-trunc-l1}, this
...
convergence in \eqref{eq:birkhoffergodic1}  is also in the
$L_p$ norm.
\end{xexercise}
...
$\textbf{Note.}$\hyperref[thm-equidistribution]{Theorem~\ref*{thm-equidistribution}} was
...
theorem (\hyperref[thm-equidistribution]{Theorem~\ref*{thm-equidistribution}}), the above
...
\eqref{eq:gauss-quotient} (which is oddly reminiscent of
...
\noindent Note that \eqref{eq:cont-frac-entropy2} and
\eqref{eq:cont-frac-entropy3} follow easily by combining
\eqref{eq:cont-frac-entropy1} with
\eqref{eq:cont-frac-approx}. The interesting constant
...
\begin{xexercise}
Prove \hyperref[lem-entropy-properties]{Lemma~\ref*{lem-entropy-properties}}.
\end{xexercise}
...
may be ignored --- see \texorpdfstring{\hyperref[exer-uniquely-decodable]{Exercise~\ref*{exer-uniquely-decodable}}}{Exercise} at the end of this section
...
property 4 in \hyperref[lem-entropy-properties]{Lemma~\ref*{lem-entropy-properties}}.
\end{proof}
...
\eqref{eq:aep-typicalevent}. For property 2, observe that
if
...
we get \eqref{eq:aep-prob-bound}. On the other hand, the
...
random variable, and explain why this implies \texorpdfstring{\hyperref[lem-entropy-ergodic]{Lemma~\ref*{lem-entropy-ergodic}}}{Lemma}.
\end{xexercise}

\noindent We refer to $H$ in \eqref{eq:entropy-ergodic} as \textbf{the
...
gives \eqref{eq:aep-lemma}.
\end{proof}

\begin{proof}[Proof of \texorpdfstring{\hyperref[thm-shannon-mcmillan-breiman]{Theorem~\ref*{thm-shannon-mcmillan-breiman}}}{Theorem}]
...
(\hyperref[eq:levy-martingale-conv]{Theorem~\ref*{eq:levy-martingale-conv}}), we have
...
\eqref{eq:psymbols1} and \eqref{eq:psymbols2})
...
\eqref{eq:entropy-newformula}.

Having derived the representation
\eqref{eq:entropy-simpler} for the source entropy, we now
...
\eqref{eq:psymbols-generalized}). So we conclude that
...
\eqref{eq:shannon-mcmillan-breiman} that we are trying to
prove, and indeed, we can deduce ``half'' of our
result from it --- a one-sided asymptotic bound ---
using \hyperref[lem-aep-lemma]{Lemma~\ref*{lem-aep-lemma}}, as follows. Define a
...
\noindent So, we are in the situation described in \texorpdfstring{\hyperref[lem-aep-lemma]{Lemma~\ref*{lem-aep-lemma}}}{Lemma}, and we conclude that almost surely we
have the inequality
...
\eqref{eq:smb-first-half} and \eqref{eq:smb-second-half}
together imply \eqref{eq:shannon-mcmillan-breiman}. To
...
\eqref{eq:aep-ergodic-average1}, namely
...
\noindent which proves \eqref{eq:smb-second-half} and thus finishes
the proof.
\end{proof}
